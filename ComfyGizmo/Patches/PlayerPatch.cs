...
namespace ComfyGizmo
{
    ...
    internal static class PlayerPatch
    {
        ...
        private static void UpdatePlacementGhostPrefix(Player __instance)
        {
        ...
        private static void UpdatePlacementPostfix(Player __instance, bool takeInput, float dt)
        {
            ...
            if (ZInput.GetKeyDown(SnapDivisionIncrementKey.Value.MainKey)) RotationManager.IncreaseSnapDivisions();

            if (ZInput.GetKeyDown(SnapDivisionDecrementKey.Value.MainKey)) RotationManager.DecreaseSnapDivisions();
        ...
        [HarmonyTranspiler]
        [HarmonyPatch(nameof(Player.UpdatePlacementGhost))]
        private static IEnumerable<CodeInstruction> UpdatePlacementGhostTranspiler(
            IEnumerable<CodeInstruction> instructions)
        {
            return new CodeMatcher(instructions)
                ...
                .Advance(5)
                .InsertAndAdvance(Transpilers.EmitDelegate<PlaceRotationDelegateDelegate>(PlaceRotationDelegate))
        ...
        private delegate Quaternion PlaceRotationDelegateDelegate(Quaternion rotation);
    }
}
