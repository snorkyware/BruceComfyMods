...
namespace ComfyGizmo
{
    ...
    public sealed class RotationManager
    {
        ...
        {
            if (!ResetRotationOnSnapDivisionChange.Value) return;
        ...
        {
            if (ResetRotationOnModeChange.Value) ResetRotation();
        ...
        {
            if (IsRoofModeEnabled.Value && RoofRotator.IsCornerRoofPieceSelected()) return _roofRotator;

            if (IsLocalFrameEnabled.Value) return _localFrameRotator;

            if (IsOldRotationEnabled.Value) return _internalRotator;
        ...
        {
            if (SnapDivisions.Value * 2 > MaxSnapDivisions) return;
        ...
        {
            if (MessageHud.m_instance) MessageHud.m_instance.ShowMessage(MessageHud.MessageType.TopLeft, message);
        }
    }
}
