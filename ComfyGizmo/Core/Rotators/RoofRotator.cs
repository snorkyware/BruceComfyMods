...
namespace ComfyGizmo
{
    ...
    {
        public static readonly HashSet<string> RoofCornerPieceNames = new()
        {
            "wood_roof_ocorner",
        ...
        public override void Rotate(Vector3 rotationAxis)
        {
            ...
            var tiltedAxis = ConvertAxisRoofMode(rotationAxis);
        ...
        {
            if (axis == Vector3.up) _eulerAngles.y = 0f;

            if (axis == Vector3.right) _eulerAngles.x = 0f;

            if (axis == Vector3.forward) _eulerAngles.z = 0f;
